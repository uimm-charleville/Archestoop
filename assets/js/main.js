...
addForm.addEventListener("submit", async (e) => {
  ...
  if (addForm.checkValidity() === false) {
    ...
    const data = await fetch("../../content/src/action.php", {
      method: "POST",
      body: formData,
    });
...
const fetchAllUsers = async () => {
  const data = await fetch("../../content/src/action.php?read=1", {
    method: "GET",
  });
  const response = await data.text();
...
const editUser = async (id) => {
  const data = await fetch(`../../content/src/action.php?edit=1&id=${id}`, {
    method: "GET",
  });
...
updateForm.addEventListener("submit", async (e) => {
  ...
  if (updateForm.checkValidity() === false) {
    ...
    showAlert.innerHTML = response;
    document.getElementById("edit-user-btn").value = "Ajouter l'utilisateur";
    updateForm.reset();
...
const deleteUser = async (id) => {
  const data = await fetch(`../../content/src/action.php?delete=1&id=${id}`, {
    method: "GET",
  });
  const response = await data.text();
  showAlert.innerHTML = response;
